...
void init_train_iris_data(float *x, float *y) {
	const int num_inputs = 120;
	const int input_size = 4;

	memcpy(x, &x_train[0][0], num_inputs * input_size * sizeof(float));
	memcpy(y, &y_train[0], num_inputs * sizeof(float));
}

void init_test_iris_data(float *x, float *y) {
	const int num_inputs = 30;
	const int input_size = 4;

	memcpy(x, &x_test[0][0], num_inputs * input_size * sizeof(float));
	memcpy(y, &y_test[0], num_inputs * sizeof(float));
}
...
int main(int argc, char** argv) {
	...
	init_train_iris_data(x.data, y.data);

	DMatrix x_val, y_val;
	x_val.row = num_test;
	x_val.col = input_size;
	x_val.data = (float *)malloc(num_test * input_size * sizeof(float));
	memset(x_val.data, 0, num_test * input_size * sizeof(float));

	y_val.row = 1;
	y_val.col = num_test;
	y_val.data = (float *)malloc(num_test * sizeof(float));
	memset(y_val.data, 0, num_test * sizeof(float));

	init_test_iris_data(x_val.data, y_val.data);

	TwoLayerNets *net = new TwoLayerNets(input_size, hidden_size, 
                                 	 	 num_classes, 1e-2);

	net->train(x, y, x_val, y_val, 1e-2, 0.95, 5e-6, 2000);
